...
export function AiMascot({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const reach = Math.min(1, distance / Math.max(80, rect.width * 2.6));
      gazeX.set((dx / distance) * 3.5 * reach);
      gazeY.set((dy / distance) * 3 * reach);
    };
  ...
  return (
    <motion.div
      ...
      whileHover={reducedMotion ? undefined : { scale: 1.06, y: -4 }}
      whileTap={{ scale: .94 }}
      onPointerDown={() => {
        setPressed(true);
    ...
    >
      <motion.div className={styles.shadow} animate={reducedMotion ? undefined : { scaleX: [1, .83, 1], opacity: [.22, .14, .22] }} transition={{ duration: 3.8, repeat: Infinity, ease: "easeInOut" }} />
      ...
      <motion.div className={styles.antenna} animate={reducedMotion ? undefined : { rotate: [-2, 3, -2] }} transition={{ duration: 2.6, repeat: Infinity, ease: "easeInOut" }}>
        <motion.i className={styles.antennaTip} animate={mood === "excited" && !reducedMotion ? { scale: [1, 1.28, 1] } : undefined} transition={{ duration: .8, repeat: Infinity }} />
      </motion.div>
      ...
      >
        <div className={styles.face}>
          <motion.i className={styles.eye} animate={{ scaleY: blinking ? .08 : mood === "excited" ? 1.13 : 1 }} transition={{ duration: .07 }}>
