...
type BrowserAgentWorkbenchProps={view?:"page"|"dom"|"a11y";highlight?:string;actions?:BrowserAction[];loggedIn?:boolean};

export function BrowserAgentWorkbench({view="page",highlight="",actions=[],loggedIn=true}:BrowserAgentWorkbenchProps){
 ...
 return <div className={styles.wrap}>
  ...
  <div className={styles.body}>
   ...
   <section className={styles.viewport}>
    {view==="page"&&<div className={styles.page}><header><b>Meetly</b><span>{loggedIn?"Ana · signed in":"Sign in"}</span></header><main><small>BOOK A CALL</small><h3>Project review</h3><label>Name<input value="Ana" readOnly/></label><label>Date<input value="2026-09-05" readOnly/></label><label>Time<select value="10:30" disabled aria-label="Selected meeting time"><option>10:30</option></select></label><motion.button className={highlight==="book"?styles.highlight:""} animate={highlight==="book"?{scale:[1,1.04,1]}:{}} transition={{duration:1.2,repeat:highlight==="book"?Infinity:0}}>Book meeting</motion.button><div className={`${styles.notice} ${highlight==="notice"?styles.highlight:""}`}>No booking created yet.</div></main></div>}
